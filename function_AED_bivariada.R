...
AED_biv <- function(Base, Response_var, Analyse = "Pre") {
  ...
  only.y <- Base[, get(glue("Response_var")), drop = FALSE]
  only.x <-
  ...
  if (Analyse == "Pre") {
    for (i in 1:(length(Base))) {
      if (x[i, 2] %in% c("character", "factor")) {
        ...
        prop1 <-
          Base[!is.na(Base[i]), ] %>% # PROPENS?O POR CATEGORICAS INICIALMENTE
          group_by(get(vars[i])) %>%
        ...
        p1 <-
          ggplot(data = filter(prop1, Response_var == 1), aes(x = get(vars[i]), y = prop)) +
          geom_bar(stat = 'identity',
                   position = 'dodge',
                   alpha = 2 / 3)  +
          geom_hline(
            yintercept = mean(select(Base, Response_var)[1:nrow(select(Base, Response_var)), ]),
            col = "red",
            linetype = 'dashed'
          ) +
  ...
  ####### pre e pos binning
    ...
    for (i in 1:(length(Base) / 2 - .5)) {
      if (x[i, 2] %in% c("character", "factor")) {
        ...
        p3 <-
          ggplot(data = prop2[prop2$Response_var == 1, ], aes(x = get(paste(
            glue("{vars[i]}.binned")
          )), y = prop)) +
          geom_bar(stat = 'identity',
                   position = 'dodge',
        ...
        p3.3 <-
          ggplot(data = Base, aes(x = get(paste(
            glue("{vars[i]}.binned")
          )))) +
        ...
        p2 <-
          ggplot(data = prop3[prop3$Response_var == 1, ], aes(x = get(glue(
            "{vars[i]}.binned"
          )), y = prop)) +
          geom_bar(stat = 'identity',
                   position = 'dodge',
