...
bin_var <- function(df, y){
  ...
  only.fctrs <- df[ , !(names(model_full_sample) %in% names(only.y))] %>% as.data.frame()  
  ...
  for(i in 1:(length(df2)-1)) {
    ...
    ### When var is factor, tree.bins will be used
      ...
      bin <- tree.bins(df2[,c(i,length(df2))], bin.nm = paste0("bin_",names(df2)[i]), y = Response, return = "lkup.list" )
  
      aux1<-as.data.frame(df[,i])
      colnames(aux1) <- names(df)[i]
      aux <- left_join(aux1,as.data.frame(bin),by=names(bin[[1]])[1])
      colnames(aux) <- c(names(bin[[1]])[1],paste0(names(bin[[1]])[1],".binned"))
      vars_bin <- cbind(vars_bin,aux[,2])
      names(vars_bin)[length(vars_bin)] = names(aux)[2]
    }
    else {
      ### When is numeric or integer, tree.bins will be used
      woe <- woe.binning(df2,  names(df2)[length(df2)],  names(df2)[i] )
